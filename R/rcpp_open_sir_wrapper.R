...
get_time <- function(days, step){
  time <- seq(0, days, 1 / step)
  time[-length(time)]
}
...
set_time <- function(out, days, step){
  month_length <- c(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31)
  time <- get_time(days, step)

  out %>%
    dplyr::mutate(t = time,
           day = rep(rep(1:365, each = step), length.out = n()),
           week = rep(rep(1:52, each = step * 7), length.out = n()),
           month = rep(rep(1:12, step * month_length), length.out = n()),
           year = rep(rep(1:1000, each = step * 365), length.out = n()))
}
